async function loadDashboard() {
  ...
  const renderSignal = (sig) => {
    ...
    return `
      <div class="signal">
        ...
        <div class="row"><span class="k">TP zone</span><span>${sig.tp_zone ? 'yes' : 'no'}</span></div>
        ...
        <div class="pipeline">
          ...
          <span class="badge ${badge(p.liquidity)}">liquidity</span>
          <span class="badge ${badge(p.zone)}">zone</span>
  ...
  document.getElementById('signals').innerHTML = (data.signals || []).map(renderSignal).join('');
}
