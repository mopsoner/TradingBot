...
## Remarque
Le dépôt inclut maintenant une implémentation Python exécutable du pipeline OpenClaw sous `src/openclaw/` (mode research/paper/live avec garde-fous risque/backtest).
...
```bash
# Research (analyse seulement)
PYTHONPATH=src python -m openclaw.cli --mode research

# Paper (exécution simulée, approval risque requis)
PYTHONPATH=src python -m openclaw.cli --mode paper --risk-approval

# Live (approval risque + backtest requis)
PYTHONPATH=src python -m openclaw.cli --mode live --risk-approval --backtest-approval
```

Les décisions sont journalisées dans `data/trade_journal.jsonl`.
