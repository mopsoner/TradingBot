...
# ─────────────────────────────────────────────────────────────────────────────
# Real SMC/Wyckoff scan on actual ETHUSDT candles
# ─────────────────────────────────────────────────────────────────────────────
...
class RealScanRequest(BaseModel):
    ...
    step_bars: int = Field(default=4, ge=1, le=48)


_real_scan_progress: dict = {}
_real_scan_lock = threading.Lock()


def _run_real_scan_bg(req: RealScanRequest) -> None:
    from backend.app.services.smc_scanner import SMCScanner, Bar

    with _real_scan_lock:
        _real_scan_progress.clear()
        _real_scan_progress.update({"status": "running", "pct": 0, "found": 0, "msg": "Chargement des bougies…"})
    ...
    try:
        ...
        with Session(engine) as s:
            ...
            bars_4h_rows = s.exec(
                select(MarketCandle)
                .where(MarketCandle.symbol == req.symbol)
                .where(MarketCandle.timeframe == "4h")
                .where(MarketCandle.timestamp >= cutoff)
                .order_by(MarketCandle.timestamp.asc())
            ).all()

        def to_bar(r: MarketCandle) -> Bar:
            ts = r.timestamp
        ...
        bars_1h = [to_bar(r) for r in bars_1h_rows]
        bars_4h = [to_bar(r) for r in bars_4h_rows]

        with _real_scan_lock:
            _real_scan_progress.update({"pct": 10, "msg": f"{len(bars_1h)} bougies {req.timeframe} chargées. Scan en cours…"})

        if len(bars_1h) < 100:
            with _real_scan_lock:
                _real_scan_progress.update({"status": "error", "msg": f"Pas assez de bougies ({len(bars_1h)} < 100)"})
            return

        # Build scanner from profile params
        scanner = SMCScanner(
            disp_threshold=float(profile_params.get("displacement_threshold", 0.55)),
            atr_min=float(profile_params.get("displacement_atr_min", 1.2)),
            ...
            fib_levels=list(profile_params.get("fib_levels", [0.5, 0.618, 0.786])),
            fib_split=bool(profile_params.get("fib_entry_split", True)),
            htf_required=bool(profile_params.get("htf_alignment_required", True)),
            vol_adaptive=bool(profile_params.get("volume_adaptive", True)),
            vol_mult_active=float(profile_params.get("volume_multiplier_active", 1.8)),
            vol_mult_off=float(profile_params.get("volume_multiplier_offpeak", 1.25)),
            allow_weekend=bool(profile_params.get("allow_weekend_trading", False)),
            risk_per_trade=float(profile_params.get("risk_per_trade", 0.01)),
            tp_rr=float(profile_params.get("take_profit_rr", 2.5)),
            stop_logic=str(profile_params.get("stop_logic", "structure")),
        )

        signals = scanner.scan(bars_1h, bars_4h, step_bars=req.step_bars)

        with _real_scan_lock:
            _real_scan_progress.update({"pct": 80, "msg": f"{len(signals)} signaux détectés. Persistance…"})

        # Persist signals and simulated trades
        saved_signals = 0
        saved_trades = 0
        with Session(engine) as s:
            # Clear previous real-scan signals for this symbol
            s.exec(select(Signal).where(Signal.symbol == req.symbol).where(Signal.setup_type.like("SMC-Real%")))  # type: ignore

            for sig in signals:
                steps_json = json.dumps([{"name": f"Étape {i+1}", "passed": st.passed, "detail": st.detail} for i, st in enumerate(sig.steps)])
                db_sig = Signal(
                    symbol=sig.symbol,
                    timeframe=sig.timeframe,
                    setup_type=f"SMC-Real/{sig.wyckoff_event}",
                    liquidity_zone="EQH" if sig.direction == "SHORT" else "EQL",
                    sweep_level=sig.sweep_level,
                    bos_level=sig.bos_level,
                    fib_zone=f"{sig.fib_level:.3f} | E:{sig.entry_price:.2f} SL:{sig.sl_price:.2f} TP:{sig.tp_price:.2f}",
                    accepted=sig.accepted,
                    direction=sig.direction,
                    wyckoff_event=sig.wyckoff_event,
                    fake_breakout=True,
                    equal_highs_lows=True,
                    expansion=True,
                    tf_4h_structure=sig.tf_4h_structure,
                    session_name=sig.session,
                    timestamp=sig.timestamp,
                    displacement_force=sig.rr_ratio,
                )
                s.add(db_sig)
                s.flush()
                saved_signals += 1
                ...
                sig_idx = next((i for i, b in enumerate(bars_1h) if b.timestamp >= sig.timestamp), None)
                if sig_idx is not None:
                    future = bars_1h[sig_idx: sig_idx + 200]
                    outcome = "OPEN"
                    for fb in future:
                        if sig.direction == "LONG":
                            if fb.low <= sig.sl_price:
                                outcome = "LOSS"
                                break
                            if fb.high >= sig.tp_price:
                                outcome = "WIN"
                                break
                        else:
                            if fb.high >= sig.sl_price:
                                outcome = "LOSS"
                                break
                            if fb.low <= sig.tp_price:
                                outcome = "WIN"
                                break
            ...
            s.add(Log(level="INFO", message=f"Real scan {req.symbol}: {saved_signals} signaux, {saved_trades} trades simulés sur {req.days}j"))
            s.commit()

        # Compute win rate
        wins = sum(1 for sig in signals if sig.accepted)
        with _real_scan_lock:
            _real_scan_progress.update({
                "status": "done",
                "pct": 100,
                "found": saved_signals,
                "trades": saved_trades,
                "msg": f"✅ Terminé — {saved_signals} signaux, {saved_trades} trades simulés",
                "days": req.days,
                "symbol": req.symbol,
                "timeframe": req.timeframe,
            })
        ...
        logger.exception("Real scan error: %s", exc)
        with _real_scan_lock:
            _real_scan_progress.update({"status": "error", "msg": f"Erreur: {exc}"})


@router.post("/data/real-scan")
def trigger_real_scan(req: RealScanRequest) -> dict:
    """Launch real SMC/Wyckoff scan on actual DB candles in background."""
    t = threading.Thread(target=_run_real_scan_bg, args=(req,), daemon=True)
    t.start()
    return {"ok": True, "symbol": req.symbol, "days": req.days}


@router.get("/data/real-scan/status")
def real_scan_status() -> dict:
    with _real_scan_lock:
        return dict(_real_scan_progress)
