...
function SymbolPicker({ enabled, onChange }: { enabled: string[]; onChange: (syms: string[]) => void }) {
  const { data: byQuote } = useApi(() => api.symbolsByQuote());
  const [activeQ, setActiveQ] = useState('USDT');
  const [search, setSearch] = useState('');

  const quotes = Object.keys(byQuote ?? { USDT: [], USDC: [], BTC: [] });
  const allSyms: string[] = (byQuote ?? {})[activeQ] ?? [];
  const filtered = search
    ? allSyms.filter(s => s.toLowerCase().includes(search.toLowerCase()))
    : allSyms;
  ...
  function toggleAll() {
    const allInQuote = allSyms;
    const allOn = allInQuote.every(s => enabledSet.has(s));
    const next = new Set(enabledSet);
    if (allOn) allInQuote.forEach(s => next.delete(s));
    else allInQuote.forEach(s => next.add(s));
    onChange(Array.from(next));
  }

  const activeCount = allSyms.filter(s => enabledSet.has(s)).length;
    ...
    <div>
      <div style={{ display: 'flex', gap: 8, marginBottom: 10, flexWrap: 'wrap', alignItems: 'center' }}>
        <div style={{ display: 'flex', gap: 4 }}>
          {quotes.map(q => (
            <button key={q} onClick={() => setActiveQ(q)}
              className={`btn ${activeQ === q ? 'btn-primary' : 'btn-secondary'}`}
        ...
        <input
          ...
          style={{ flex: 1, minWidth: 120, padding: '4px 10px', fontSize: 12, borderRadius: 6, border: '1px solid var(--border)', background: 'var(--surface)', color: 'var(--text)' }}
        />
        <button className="btn btn-secondary" onClick={toggleAll} style={{ fontSize: 11, padding: '4px 10px', whiteSpace: 'nowrap' }}>
          {activeCount === allSyms.length ? 'Tout désactiver' : 'Tout activer'}
        </button>
      </div>
      <div style={{ fontSize: 11, color: 'var(--text-muted)', marginBottom: 8 }}>
        {enabled.length} symbole{enabled.length !== 1 ? 's' : ''} activé{enabled.length !== 1 ? 's' : ''} au total · {activeCount}/{allSyms.length} en {activeQ}
      </div>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(110px, 1fr))', gap: 6, maxHeight: 280, overflowY: 'auto', padding: '2px 0' }}>
        {filtered.map(sym => {
          const on = enabledSet.has(sym);
          return (
            <label key={sym} onClick={() => toggle(sym)}
              style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '5px 8px', borderRadius: 6, cursor: 'pointer', border: `1px solid ${on ? 'var(--accent)' : 'var(--border)'}`, background: on ? 'rgba(59,130,246,0.12)' : 'var(--surface2)', userSelect: 'none', transition: 'all 0.15s' }}>
              <span style={{ width: 12, height: 12, borderRadius: 3, border: `1.5px solid ${on ? 'var(--accent)' : 'var(--border)'}`, background: on ? 'var(--accent)' : 'transparent', flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                {on && <span style={{ color: '#000', fontSize: 9, fontWeight: 900 }}>✓</span>}
              </span>
              <span style={{ fontSize: 11, fontWeight: on ? 600 : 400, color: on ? 'var(--accent)' : 'var(--text)' }}>{fmtSym(sym)}</span>
            </label>
          );
        })}
        {filtered.length === 0 && <div style={{ color: 'var(--text-muted)', fontSize: 12, gridColumn: '1/-1', padding: 8 }}>Aucun symbole trouvé</div>}
      </div>
    </div>
  );
}
