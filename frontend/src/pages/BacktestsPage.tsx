...
export function BacktestsPage() {
  ...
  const runBacktest = async () => {
    ...
    setStatus(`Erreur: ${String(res.reason)} ${res.detail ? `- ${String(res.detail)}` : ''}`);
  };

  const openReport = async (backtestId: number) => {
    const res = await api.backtestReport(backtestId);
    if (!res.ok) {
      setStatus(`Impossible de charger le rapport: ${String(res.reason)}`);
      return;
    }
    setReport(String(res.report));
    setStatus(`Rapport #${backtestId} chargé.`);
  };

  const deleteReport = async (backtestId: number) => {
    const res = await api.deleteBacktest(backtestId);
    if (!res.ok) {
      setStatus(`Suppression impossible: ${String(res.reason)}`);
      return;
    }
    setStatus(`Rapport #${backtestId} supprimé.`);
    reload();
  };
  ...
  return (
    <section>
      ...
      <div className="card" style={{ marginTop: 16 }}>
        <h3>Manager les rapports</h3>
        <table>
          <thead><tr><th>Date</th><th>Symbol</th><th>TF</th><th>Strategy</th><th>WR</th><th>PF</th><th>Actions</th></tr></thead>
          <tbody>
            {rows.map((r) => (
              <tr key={r.id}>
                ...
                <td>
                  <div className="flex gap-8">
                    <button className="btn btn-secondary" onClick={() => openReport(r.id)}>Voir</button>
                    <button className="btn" onClick={() => deleteReport(r.id)}>Supprimer</button>
                  </div>
                </td>
              </tr>
            ))}
